...
export FILES=(
	...
	# # JS Files
	# #################
	# "./web/js/navigation-menu"
	# "./Magento_Theme/web/js/theme"
	# "./Magento_Theme/web/js/responsive"
	# "./Magento_Theme/requirejs-config"
...
if [[ "$1" == "convert" ]]; then
	...
	for file in "${FILES[@]}"
	do
		if [ -f "$file.$src" ] && [ ! -f "$file.$dest" ]; then
...
if [[ "$1" == "hide" ]]; then
	...
	do
		# + less
		# - tmp
			if [ -f "$file.less" ] && [ ! -f "$file.less.tmp" ]; then
				mv "$file.less" "$file.less.tmp"
				echo "✅  $file"
			else
				echo "❌  $file"
			fi
...
if [[ "$1" == "find" ]]; then
	[[ "$2" == "m" || "$3" == "m" ]] && missing=true

# try using fall back var
# BASE_URL=${REPLY_URL:-$DEFAULT_URL}
	...
	[[ "$kind" != ".less" ]] && kind=".less$kind"

	if [[ "$missing" ]]; then
		echo "Missing [$kind] files:"
		for file in "${FILES[@]}"
		do
			[[ ! -f "$file$kind" ]] && echo "$file$kind"
		done
	else
		echo "Found [$kind] files:"
		for file in "${FILES[@]}"
		do
			[[ -f "$file$kind" ]] && echo "$file$kind"
		done
